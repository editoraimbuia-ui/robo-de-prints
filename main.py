...
def executar_prints():
    ...
    with sync_playwright() as p:
        ...
        for item in campanhas:
            ...
            d_inicio = converter_data(str(item.get("data_inicio", "")))
            d_fim = converter_data(str(item.get("data_fim", "")))
            ...
            if status == "ATIVO" and url and dentro_do_prazo:
                ...
                # Tenta recarregar até 5 vezes por causa de banners rotativos
                for tentativa in range(5):
                    try:
                        print(f"Tentativa {tentativa+1} para {cliente} em {url}")
                        ...
                        # Injeta a simulação do Navegador + Barra do Windows no print
                        page.evaluate(f"""
                            () => {{
                        ...
                        nome_arquivo = f"prints/{cliente_limpo}_{data_arquivo}.png"
                ...
                if not sucesso:
                    falhas.append(f"Cliente: {cliente} | URL: {url}")

        browser.close()

    if capturas_realizadas == 0 and not falhas:
        with open("prints/aviso.txt", "w") as f:
            f.write("Nenhuma campanha ATIVA e DENTRO DO PRAZO foi encontrada hoje.")

    if falhas:
        print("::: ATENÇÃO: FALHA NOS PRINTS ABAIXO :::")
        for f in falhas:
            print(f)
        raise Exception("Aviso: Alguns prints falharam.")

if __name__ == "__main__":
    executar_prints()import os
import requests
from datetime import datetime
from playwright.sync_api import sync_playwright
